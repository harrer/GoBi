...
public class Superposition {
    ...
    public static Object[] superimpose(DoubleMatrix2D P, DoubleMatrix2D Q, DoubleMatrix2D Q_full, boolean full){
        ...
        Q = full ? move_Q_onto_P(Q_full, R, t) : move_Q_onto_P(Q, R, t);
        return new Object[]{Q, readOffRMSD(P, Q), RMSD(covar, initError(cP, cQ), P.rows()), gdt_ts(P, Q)};
    }
    
    private static DoubleMatrix1D getCentroid(DoubleMatrix2D matrix){
    ...
    private static DoubleMatrix2D translate(DoubleMatrix2D matrix, DoubleMatrix1D centroid){
    ...
    private static double initError(DoubleMatrix2D cP, DoubleMatrix2D cQ){
        double error = 0;
...
//        DoubleMatrix2D cQ = alg.mult(centroid, q);
    ...
    private static DoubleMatrix2D covarMatrix(DoubleMatrix2D p, DoubleMatrix2D q){
        Algebra alg = new Algebra();
        //return alg.mult(alg.transpose(p), q);
        return alg.mult(alg.transpose(p), q);
    }
    
    private static DoubleMatrix2D rotate(DoubleMatrix2D covarMatrix){
    ...
    private static double RMSD(DoubleMatrix2D covarMatrix, double E0, double L){
        SingularValueDecomposition svd = new SingularValueDecomposition(covarMatrix);
        Algebra alg = new Algebra();
    ...
    private static DoubleMatrix1D T(DoubleMatrix2D R, DoubleMatrix1D c_p, DoubleMatrix1D c_q){
        DoubleMatrix1D t;
        Algebra alg = new Algebra();
    ...
    private static DoubleMatrix2D move_Q_onto_P(DoubleMatrix2D Q, DoubleMatrix2D R, DoubleMatrix1D T){// hier kann gesamte pdb Koordinaten verwendet werden
        R = R.viewDice();
        int r = Q.rows();
    ...
    private static double readOffRMSD(DoubleMatrix2D P, DoubleMatrix2D QontoP){
        double distance = 0;
    ...
    public static double gdt_ts(DoubleMatrix2D P, DoubleMatrix2D Q){
